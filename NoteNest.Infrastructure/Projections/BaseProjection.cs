...
namespace NoteNest.Infrastructure.Projections
{
    ...
    public abstract class BaseProjection : IProjection
    {
        ...
        protected async Task<SqliteConnection> OpenConnectionAsync()
        {
            ...
            // ✅ CRITICAL: Ensure synchronous = FULL for guaranteed durability
            // In DELETE journal mode, this ensures rollback journal is fsynced before commit
            // Provides immediate persistence to main DB file (no delayed checkpoint)
